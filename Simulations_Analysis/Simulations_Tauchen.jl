...
include(path * "/" * method * "_Tauchen" * "/2 states" * "/" * "BY_" * method *"_Tauchen.jl")
...
for k in 1:K
    ...
    #PD ratio in levels
    pd_num=Array{Float64}(undef,lastindex(σ_range),1)
    #Define a function to interpolate from the policy function of the PD ratio
    #we need to know which value of the grid is the closest to the actual value of the volatility
    #as we are going to pick the corresponding grid for x
    indices=[findmin(abs.(σ_grid .- σ_range[i]))[2][1] for i in 1: lastindex(σ_range)]
    for i in 1:lastindex(σ_range)
        x_temp=x_range[i]
        σ_temp=σ_range[i]

        #avoid extrapolation
        if x_temp>maximum(x_grid[indices[i],:])
            x_temp=maximum(x_grid[indices[i],:])
        elseif x_temp<minimum(x_grid[indices[i],:])
            x_temp=minimum(x_grid[indices[i],:])
        else
        end

        if σ_temp>maximum(σ_grid)
            σ_temp=maximum(σ_grid)
        elseif σ_temp<minimum(σ_grid)
            σ_temp=minimum(σ_grid)
        else
        end

        PD_f=linear_interpolation((vec(x_grid[indices[i],:]), vec(σ_grid)), log.(pd_pf_wc))
        pd_num[i]=exp(PD_f(x_temp, σ_temp))
    end
    ...
    Rf_num=Array{Float64}(undef,lastindex(σ_range),1)
    #risk free rate
    for i in 1:lastindex(σ_range)
        x_temp=x_range[i]
        σ_temp=σ_range[i]
        ...
        PD_f=linear_interpolation((vec(x_grid[indices[i],:]), vec(σ_grid)), Rf)
        Rf_num[i]=PD_f(x_temp, σ_temp)
    end 
...
if simulation_type=="population"
    ...
    CSV.write("returns_" * method * "_Tauchen.csv",returns_df) #save in a CSV file
else
end
