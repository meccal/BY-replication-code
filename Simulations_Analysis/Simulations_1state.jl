...
include(path * "/" * method * "/1 state" * "/" * "BY_" * method *"_1state.jl")
...
for k in 1:K
    ...
    #PD ratio in levels
    pd_num=[Ch_pol(ζ, x_range_Ch[i], n_x) for i in 1:lastindex(x_range)]
    ...
    #risk free rate
    Rf_num=log.([(β^θ*exp(-γ*(μ_c+x_range[i])+0.5*γ^2*σ^2)*(Ch_pol(α, x_range_Ch[i], n_x)-1)^(1-θ)*
    GH_WC(α, n, n_x, x_min, x_max,  σ*ϕ_e, ρ*x_range[i], nodes, weights, θ-1))^(-1) for i in 2:lastindex(x_range)])
...
if simulation_type=="population"
    ...
    CSV.write("C_D_1 state.csv",C_D_df) #save in a CSV file
    ...
    CSV.write("returns_" * method * "_1state.csv",returns_df) #save in a CSV file
else
end
